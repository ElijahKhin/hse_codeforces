...
static int query(std::vector<std::vector<int>>& sparseTree, std::vector<int>&arr, int l, int r) {
	...
	int idx1 = sparseTree[l][p];
	int idx2 = sparseTree[r - (1 << p) + 1][p];
...
static void preprocessSparseTree (
  ...
  for (int k = 1; k < max_p; ++k) {
		for (int i = 0; i + (1 << k) <= n; ++i) {
			int idx1 = sparseTree[i][k - 1];
  		int idx2 = sparseTree[i + (1 << (k - 1))][k - 1];
  		sparseTree[i][k] = (arr[idx1] >= arr[idx2]) ? idx1 : idx2;
  	}
  }
...
int main() {
  ...
  std::cin >> n;
	...
	for (int i = 0; i < k; ++i) {
		...
		--l;
		--r;
		std::cout << query(sparseTree,arr, l, r)+1 << ' ';
	}
	std::cout << '\n';
